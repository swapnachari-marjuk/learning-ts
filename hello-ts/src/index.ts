...
const lastName: string = "Marjuk"; // example of explicit typing.

// typescript primary types are: string, number, boolian...
// firstName = 3; this will be an error in typescript.

const greets = (name: string, age: number): string => {
...
const voidGreets = (name: string, age: number): void => {
  const greetsSent = `Hello! I'm ${name}. I'm ${age} years old.`;
  console.log(greetsSent);
};
...
const devs: { name: string; skilled: boolean; age: number; isMarid?: boolean } =
  {
    name: "marjuk",
    skilled: false,
    age: 20,
...
// same vabe amra interface diye o type declare kore dite pari.
